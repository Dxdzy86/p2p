...
<script type="text/javascript">
	$(function() {
		...
		$(".auditClass").click(function(){
			...
			$("input[name='id']").val(jsonString.id);
			$("#username").text(jsonString.username);
			$("#realName").text(jsonString.realName);
			$("#sex").text(jsonString.sex);
			$("#birthDate").text(jsonString.birthDate);
			$("#address").text(jsonString.address);
			$("#idNumber").text(jsonString.idNumber);
			$("#image1").attr("src",jsonString.image1);
			$("#image2").attr("src",jsonString.image2);
		...
		$(".btn_audit").click(function(){
			...
			$("#editform").ajaxSubmit({
				success:function(data){
					if(data.success){
						$.messager.confirm("提示","您确定要审核吗？",function(){
					...
					<table class="table">
						...
						<#list pageResult.data as info>
							<tr>
								...
								<td>${info.address}</td>
								<#if (info.state == 1)>
								<td><font color="green">${info.stateDisplay}</font></td>
								<#else>
								<td><font color="red">${info.stateDisplay}</font></td>
								</#if>
