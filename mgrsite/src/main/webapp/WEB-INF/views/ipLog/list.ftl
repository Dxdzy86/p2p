...
<script type="text/javascript">
	$(function(){
		$("#loginState option[value=${(qo.loginState)!-1}]").attr("selected",true);
		$("#userType option[value=${(qo.userType)!-1}]").attr("selected",true);
			...
			<div class="col-sm-9">
				...
				<div class="panel panel-default">
					<table class="table">
						...
						<tbody id="tbody">
							<#list pageResult.data as iplog>
							<tr>
								<td>${iplog.username}</td>
								<td>${iplog.loginTime?string("yyyy-MM-dd")}</td>
						        <td>${iplog.ip}</td>
						        <td>${iplog.loginStateDisplay}</td>
						        <td>${iplog.userTypeDisplay}</td>
							</tr>
							</#list>
						</tbody>
					</table>
